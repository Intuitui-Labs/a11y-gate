...
| :-: | :--- | :--- | :--- |
| **1** | **npm Version** | Current latest stable release version | `https://img.shields.io/npm/v/@intuitui-labs/a11y-gate` |
...
| **4** | **License** | Permissive open-source commercial terms (MIT) | `https://img.shields.io/npm/l/@intuitui-labs/a11y-gate` |
| **5** | **Bundle Size (minzip)** | Client impact when bundled in production | `https://img.shields.io/bundlephobia/minzip/@intuitui-labs/a11y-gate` |
| **6** | **Monthly Downloads** | Adoption velocity and active usage | `https://img.shields.io/npm/dm/@intuitui-labs/a11y-gate` |
| **7** | **TypeScript Native** | Zero extra `@types` package needed | `https://img.shields.io/npm/types/@intuitui-labs/a11y-gate` |
| **8** | **Tree Shaking** | Pure ESM modules with `sideEffects: false` | Static verified badge |
| **9** | **APCA & WCAG 3** | W3C APCA 0.98G perceptual lightness contrast compliance | Static contract badge |
| **10** | **Vitest 5 Engine** | Tested on latest modern test runner | Static engine badge |
| **11** | **Node & Browser** | Dual runtime compatibility (Node 18+, Modern Browsers, React Native) | `https://img.shields.io/node/v/@intuitui-labs/a11y-gate` |
...
| **14** | **GitHub Stars** | Community popularity and engagement | `https://img.shields.io/github/stars/NeevSK/a11y-gate` |
| **15** | **PRs Welcome** | Community openness to issues and contributions | Static badge |

---

## 3. Implementation Blueprint for `@intuitui-labs/a11y-gate`

1. **GitHub Actions CI Workflow (`.github/workflows/ci.yml`)**:
   - Runs on all pushes and PRs to `main`.
   - Executes `pnpm check-types`, `pnpm run test:coverage`, and `pnpm build`.
   - Generates `lcov.info` and uploads coverage artifacts to Codecov.
2. **README Header Integration**:
   - Embeds clean, unified flat-square badges displaying npm version, build status, bundle size, license, APCA compliance, Vitest engine, and types.
