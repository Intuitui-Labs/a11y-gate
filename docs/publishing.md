# Publishing Guide for `@intuitui-labs/a11y-gate`

This guide details the release cycle, quality preflight checks, authentication, 2FA/OTP handling, and CI/CD automation for publishing **`@intuitui-labs/a11y-gate`** to the public npm registry.
...
- **Initial release** (`0.1.0`): Initial public release of APCA, CVD simulation, spatial harmony, and mobile ergonomics audit engine.
- **Patch release** (`0.1.0` → `0.1.1`): Bug fixes, token formula updates, non-breaking type adjustments.
- **Minor release** (`0.1.1` → `0.2.0`): Backward-compatible new features (e.g., new color space adapters, WCAG 3 drafts).
- **Major release** (`0.2.0` → `1.0.0`): Breaking API changes or mathematical formula deprecations.
...
2. **Scope**: Select your organization `@intuitui-labs` or package `@intuitui-labs/a11y-gate`.
...
1. In the GitHub repository (`https://github.com/NeevSK/a11y-gate`), navigate to **Settings** → **Secrets and variables** → **Actions**.
...
npm view @intuitui-labs/a11y-gate version
npm view @intuitui-labs/a11y-gate dist-tags

# Inspect full release metadata
npm view @intuitui-labs/a11y-gate time --json
```

You can also visit the package page directly at:
[https://www.npmjs.com/package/@intuitui-labs/a11y-gate](https://www.npmjs.com/package/@intuitui-labs/a11y-gate)
