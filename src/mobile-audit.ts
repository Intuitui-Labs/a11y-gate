...
/* ==========================================================================
   React Native Mobile Screen Qualitative & Ergonomics Audit Engine
   ========================================================================== */
 ...
 */
export function auditReactNativeTypography(sourceCode: string): ReactNativeScreenFinding | null {
  // Matches Tailwind class text-[10px], text-[11px], text-[12px], text-[13px]
  const sub14Tailwind = Array.from(
    sourceCode.matchAll(/\btext-\[(1[0-3]|[1-9])px\]/gi),
    (m) => m[0]
  );
  // Matches inline styles fontSize: 10, fontSize: 11, fontSize: 12, fontSize: 13
  const sub14Inline = Array.from(
    sourceCode.matchAll(/\bfontSize:\s*(1[0-3]|[1-9])\b/gi),
    (m) => m[0]
  );
 ...
 */
export function auditReactNativeTouchTargets(sourceCode: string): ReactNativeScreenFinding | null {
 ...
 */
export function auditReactNativeQualitativeUx(sourceCode: string): ReactNativeScreenFinding[] {
  const findings: ReactNativeScreenFinding[] = [];
 ...
 */
export function auditReactNativeScreen(
  sourceCode: string,
  screenName: string = 'Screen'
): ReactNativeScreenAuditResult {
  const findings: ReactNativeScreenFinding[] = [];
 ...
 */
export function runReactNativeScreenAuditSuite(
  screens: { name: string; sourceCode: string }[],
  testHooks: { describe: Function; it: Function; expect: Function }
): void {
  const { describe, it, expect } = testHooks;
